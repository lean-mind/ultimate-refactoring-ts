...
describe('Player should', () => {
  it('Has the given score', () => {
    const player = Player.createExistingPlayer(Role.Warrior, 'Garrosh Hellscream', 10)

    expect(player.getScore()).toBe(10)
  })

  it('Has an empty score', () => {
    const player = Player.createNewPlayer(Role.Dragon, 'Onyxia - Ruler of the Black Dragonflight')
